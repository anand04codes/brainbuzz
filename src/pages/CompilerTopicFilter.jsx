...
int main() {
  ...
  const filterQuestionsByTopic = (topic) => {
    if (topic === 'all') {
      ...
      // Smart topic filtering based on question content
      const filtered = allQuestions.filter(question => {
        ...
        switch (topic.toLowerCase()) {
          case 'array':
            return title.includes('array') || description.includes('array') ||
                   title.includes('subarray') || title.includes('two sum') ||
                   title.includes('maximum subarray');

          case 'string':
            return title.includes('string') || description.includes('string') ||
                   title.includes('reverse string') || title.includes('parentheses');

          case 'dynamic programming':
            return title.includes('climbing') || title.includes('maximum subarray') ||
                   description.includes('dynamic') || description.includes('dp');

          case 'tree':
            return title.includes('tree') || title.includes('binary') ||
                   description.includes('tree') || description.includes('binary tree') ||
                   title.includes('inorder') || title.includes('symmetric');

          case 'linked list':
            return title.includes('linked') || title.includes('merge') ||
                   description.includes('linked') || description.includes('list');

          case 'stack':
            return title.includes('parentheses') || description.includes('stack');

          case 'math':
            return title.includes('climbing') || description.includes('mathematical');

          default:
            return true; // Show all for unknown topics
        }
      });
  ...
  const executeJavaScript = (code) => {
    ...
    try {
      ...
      return {
        ...
        time: executionTime,
        memory: 'N/A',
        complexity: 'O(1)' // Simplified complexity analysis
      };
    } catch (error) {
      const endTime = performance.now();
      const executionTime = ((endTime - startTime) / 1000).toFixed(3);
  ...
  const simulateExecution = (code, language) => {
    const startTime = performance.now();
    setTimeout(() => {}, 100); // Simulate processing time
    const endTime = performance.now();
    const executionTime = ((endTime - startTime) / 1000).toFixed(3);
    ...
    return {
      output,
      time: executionTime,
      memory: Math.floor(Math.random() * 10) + 5 + 'KB',
      complexity
    };
  };
    ...
    try {
      ...
      if (selectedLanguage === 'javascript') {
        try {
          ...
          setTimeComplexity(result.complexity);
        } catch (error) {
      ...
      // For other languages, simulate execution with realistic output
      const mockResult = simulateExecution(code, selectedLanguage);
      setExecutionOutput(mockResult.output);
      setExecutionTime(mockResult.time);
      setExecutionMemory(mockResult.memory);
      setTimeComplexity(mockResult.complexity);

      setRunning(false);

    } catch (error) {
      console.error('Error executing code:', error);
      setExecutionOutput('Error executing code. Please check your code syntax.');
      setRunning(false);
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-gray-900 shadow-md rounded-lg p-4">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-bold">Question List ({filteredQuestions.length})</h3>
